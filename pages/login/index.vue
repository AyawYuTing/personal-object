<template>
	<view class="content">
        <div>
            ...
            <input type="text" v-model="login_form.psw" placeholder="请输入密码">
            <input type="text" v-model="reinput_psw" placeholder="请重复输入密码" v-if="!type">
        </div>
		...
		methods: {
            ...
            register(){
                if(!this.login_form.name){
                    ...
                    uni.showToast({ title: "请填写用户密码", icon: "none" });
                }else if(!this.reinput_psw){
                    uni.showToast({ title: "请重新确认密码", icon: "none" });
                }else if(this.reinput_psw != this.login_form.psw){
